// =====================================================================
//  Каталог вентиляторов
//  Курсовая работа: "Адаптивная многокритериальная фильтрация в структурированных каталогах с неоднородными параметрами"
//  Андреев А.А., РЭУ им. Г.В. Плеханова, 2026
// =====================================================================
//
//  Математическая модель:
//    Fan = (Q_min, Q_max, P_min, P_max, N_y, D)

//    f = f_validate * f_normalize * f_parse * f_clean


//    Π(Fan) = π1 AND π2 AND π3 AND π4 AND π5


//    r(Fan) = N_y / N_гидр,  N_гидр = (Q-·P-)/3600
//    g: D_clean -> {CORRECT, CORRECTED, NEEDS_CHECK, ERROR}
//
//  Bloom-фильтр:
//    P_fp ~~ (1 − e^(−kn/m))^k
//    Метод Кирша-Митценмахера: pos_i = (h1 + i·h2) mod m
//
// =====================================================================
...
// =====================================================================
//  РАЗДЕЛ 1: КОНФИГУРАЦИЯ И КОНСОЛЬНЫЕ УТИЛИТЫ
// =====================================================================
...
// Константы математической модели
const double R_TYPICAL_LOW  = 1.5;   // нижняя граница типичного r
const double R_TYPICAL_HIGH = 10.0;  // верхняя граница типичного r
const double R_TYPICAL_MID  = 5.0;   // середина — для выбора при неоднозначности
const double R_HARD_LOW     = 0.5;   // ниже — физическая невозможность
const double R_HARD_HIGH    = 100.0; // выше — физическая невозможность
const double R_SOFT_LOW     = 1.0;   // ниже — NEEDS_CHECK
const double R_SOFT_HIGH    = 30.0;  // выше — NEEDS_CHECK
...
const unsigned int BLOOM_K  = 3;     // число хеш-функций
...
void pressEnter()
{
    ...
    cin.ignore(1000, '\n');
    // если буфер пуст, getchar() ждёт
    // если не пуст — уже прочитали
}

// =====================================================================
//  РАЗДЕЛ 2: СТРУКТУРЫ ДАННЫХ (каноническое представление Fan)
// =====================================================================
...
// Каноническое представление Fan = (Q_min, Q_max, P_min, P_max, N_y, D)
// n не входит в Fan — это производная характеристика (хранится в метаданных)
struct Fan
{
    unsigned int id;
    string type;       // тип (конструктивная категория): ВО, ВЦ, ВКОП, ВР, Ц
    string model;
    string size;       // типоразмер (габаритный ряд внутри типа)
    unsigned short diameter;  // D — диаметр рабочего колеса, мм

    double flowMin;    // Q_min, м3/ч
    double flowMax;    // Q_max, м3/ч
    double pressureMin; // P_min, Па
    double pressureMax; // P_max, Па
    double power;      // N_y — потребляемая электрическая мощность, кВт

    double efficiency; // η — производная характеристика (метаданные)
    unsigned int price;

    string status;     // g(Fan): CORRECT | CORRECTED | NEEDS CHECK | ERROR DETECTED
    ErrorType errorType;
    bool corrected;    // были ли применены коррекции (swap, пересчёт единиц)
    bool effImputed;   // η восстановлен расчётным путём
};
...
struct Query
{
    ...
    double Q;          // точка запроса: производительность, м³/ч
    double P;          // точка запроса: давление, Па
    bool includeYellow; // включать NEEDS CHECK?
};

// =====================================================================
//  РАЗДЕЛ 3: PIPELINE — этапы f_clean, f_parse, f_normalize, f_validate
// =====================================================================
...
{
    try { if (s.empty()) return false; out = stod(s); return true; }
    catch (...) { return false; }
}

bool safeStou(const string& s, unsigned int& out)
{
    try { if (s.empty()) return false; long v = stol(s); if (v < 0) return false; out = (unsigned int)v; return true; }
    catch (...) { return false; }
}
...
bool isEmptyField(const string& s)
{
    ...
    if (t == "-" || t == "\xe2\x80\x94" || t == "?" || t == "??" || t == "???") return true;
...
// Предусловие: input — ненулевая строка после f_clean
// Постусловие: minVal ≤ maxVal (π₁/π₂ гарантированы), corrected=true если swap
bool parseRange(const string& input, double& minVal, double& maxVal, bool& corrected)
{
    ...
    if (pos != string::npos) { sMin = input.substr(0, pos); sMax = input.substr(pos + 3); found = true; }
    ...
    // ASCII '-' с цифрой слева (уже не отработает для "3240 - 21800", но нужен для случаев без пробелов)
    if (!found) {
        for (unsigned int i = 1; i < input.size(); i++) {
            if (input[i] == '-' && isdigit((unsigned char)input[i - 1])) {
    ...
    // Инвариант: гарантируем π₁ (Q_min ≤ Q_max) или π₂ (P_min ≤ P_max)
    if (a > b) { swap(a, b); corrected = true; }
    minVal = a; maxVal = b;
    return true;
}
...
// Предусловие: s — строковое значение мощности из CSV
// Постусловие: возвращает мощность в кВт, err = NONE при успехе
// При неоднозначности (оба варианта проходят P_adm) используется r для выбора
                      ...
                      double P_min, double P_max, bool& /*corrected*/, ErrorType& err)
{
    ...
    bool hasKW = (lower.find("kw") != string::npos || lower.find("\xd0\xba\xd0\xb2\xd1\x82") != string::npos);
    bool hasW  = (!hasKW) && (lower.find("w") != string::npos || lower.find("\xd0\xb2\xd1\x82") != string::npos);

    string cleaned = removeSpaces(s);
    double value;
    if (!safeStod(cleanNumber(cleaned), value)) { err = PARSE_ERROR; return 0; }

    if (hasKW) return value;
    if (hasW)  return value / 1000.0;

    // Без суффикса: предикат допустимости P_adm
    double asW  = value / 1000.0; // трактовка: ватты → кВт
    double asKW = value;          // трактовка: уже кВт

    bool okW  = (asW  >= POWER_MIN_KW && asW  <= POWER_MAX_KW);
    bool okKW = (asKW >= POWER_MIN_KW && asKW <= POWER_MAX_KW);
    ...
    if (okW && okKW) {
        ...
        double P_mid = (P_min + P_max) / 2.0;
        double N_hydr = (Q_mid * P_mid) / 3600.0; // Вт

        if (N_hydr > 0) {
            double r_W  = (asW  * 1000.0) / N_hydr;
            double r_KW = (asKW * 1000.0) / N_hydr;
            double dist_W  = fabs(r_W  - R_TYPICAL_MID);
            double dist_KW = fabs(r_KW - R_TYPICAL_MID);
            return (dist_W <= dist_KW) ? asW : asKW;
        }
        return asW; // fallback — каталожный дефолт
    }
...
// Предусловие: Fan заполнен после f_parse и f_normalize
// Постусловие: f.status ∈ {CORRECT, CORRECTED, NEEDS CHECK, ERROR DETECTED}
void validateFan(Fan& f)
{
    // Ранний выход при ошибке предыдущего этапа
    if (f.errorType != NONE) { f.status = "ERROR DETECTED"; return; }

    // π₃: Q > 0, π₄: P > 0
    if (f.flowMin <= 0 || f.flowMax <= 0 || f.pressureMin <= 0 || f.pressureMax <= 0)
    { f.errorType = INVALID_PHYSICAL_VALUE; f.status = "ERROR DETECTED"; return; }

    // π₅: N_y > 0
    if (f.power <= 0) { f.errorType = INVALID_POWER; f.status = "ERROR DETECTED"; return; }

    // Гидравлическая мощность (теоретический минимум)
    // N_гидр = (Q̄ · P̄) / 3600 [Вт]
    double Q_mid = (f.flowMin + f.flowMax) / 2.0;
    double P_mid = (f.pressureMin + f.pressureMax) / 2.0;
    double N_hydr_W = (Q_mid * P_mid) / 3600.0;

    if (N_hydr_W <= 0) { f.errorType = INVALID_STRUCTURE; f.status = "ERROR DETECTED"; return; }

    // Коэффициент согласованности: r = N_y / N_гидр
    double Ny_W = f.power * 1000.0;
    double r = Ny_W / N_hydr_W;

    // Восстановление η в двух точках диапазона (метаданные)
    double eta_min = (f.flowMin * f.pressureMin) / (Ny_W * 3600.0);
    double eta_max = (f.flowMax * f.pressureMax) / (Ny_W * 3600.0);
    f.efficiency = eta_min; // сохраняем оценку
    f.effImputed = true;
    (void)eta_max; // используется в расширенной версии для диапазонной проверки

    // Жёсткие границы
    if (r < R_HARD_LOW || r > R_HARD_HIGH)
    { f.errorType = INVALID_EFFICIENCY; f.status = "ERROR DETECTED"; return; }
...
// =====================================================================
//  РАЗДЕЛ 4: CSV ввод/вывод
// =====================================================================

string errorMessage(ErrorType e)
{
    switch (e) {
    case PARSE_ERROR:            return "E1: Parse error";
    case INVALID_RANGE:          return "E2: Invalid range";
    case INVALID_PHYSICAL_VALUE: return "E3: Physical violation";
    case INVALID_EFFICIENCY:     return "E4: Efficiency OOR";
    case INVALID_POWER:          return "E5: Invalid power";
    case INVALID_STRUCTURE:      return "E6: Structure error";
    case EMPTY_FIELD:            return "E7: Empty field";
    default:                     return "E0: OK";
    }
}

string colorStatus(const string& s) {
    if (s == "CORRECT")     return string(clr::GRN) + s + clr::RST;
    if (s == "CORRECTED")   return string(clr::YLW) + s + clr::RST;
    if (s == "NEEDS CHECK") return string(clr::YLW) + s + clr::RST;
    return string(clr::RED) + s + clr::RST;
}
...
bool loadCSV(const string& path, vector<RawFan>& out)
{
    ...
    while (getline(in, line)) {
        if (firstLine) {
            ...
            // [P2] Детектор повторной прогонки: нормализованный файл содержит Q_min/Q_max в шапке
        ...
        RawFan r;
        r.id = cols[0]; r.type = cols[1]; r.model = cols[2]; r.size = cols[3];
        r.diameter = cols[4]; r.flow = cols[5]; r.pressure = cols[6]; r.power = cols[7];
...
{
    // ios::binary — предотвращает трансформацию символов на Windows
    ...
    for (unsigned int i = 0; i < fans.size(); i++) {
        ...
        out.precision(4); out << f.power << ';' << f.efficiency << ';';
        out.unsetf(ios::fixed);
        out << f.price << ';' << f.status << ';'
            << (f.corrected ? "1" : "0") << ';' << (f.effImputed ? "1" : "0") << ';'
...
// =====================================================================
//  РАЗДЕЛ 5: ИНДЕКСНЫЕ СТРУКТУРЫ (Bloom, хеш-таблица, multimap)
// =====================================================================

// Bloom-фильтр: m=4096 бит, k=3 хеш-функции
// P_fp ≈ (1 − e^(−kn/m))^k ≈ 8.5×10⁻⁸ при n=6
struct BloomFilter
{
    vector<bool> bits;
    BloomFilter() : bits(BLOOM_M, false) {}

    // Метод Кирша-Митценмахера: pos_i = (h₁ + i·h₂) mod m
    // h₁ = hash(s), h₂ = hash(s + "_salt")
    unsigned int hashI(const string& s, unsigned int i) const {
        size_t h1 = std::hash<string>{}(s);
        size_t h2 = std::hash<string>{}(s + "_salt");
        return (unsigned int)((h1 + i * h2) % BLOOM_M);
    }

    void add(const string& s) {
        for (unsigned int i = 0; i < BLOOM_K; i++) bits[hashI(s, i)] = true;
    }

    bool maybeContains(const string& s) const {
        for (unsigned int i = 0; i < BLOOM_K; i++)
            if (!bits[hashI(s, i)]) return false; // точно нет
...
{
    idx.byType.clear(); idx.byQmin.clear(); idx.bloomTypes = BloomFilter();
    for (unsigned int i = 0; i < fans.size(); i++) {
        const Fan& f = fans[i];
...
// =====================================================================
//  РАЗДЕЛ 6: ПОИСК (линейный + индексный)
// =====================================================================

// Предикат соответствия: запрос = точка, запись = диапазон
inline bool matches(const Fan& f, const Query& q)
{
    if (f.status == "ERROR DETECTED") return false;
    if (!q.includeYellow && f.status == "NEEDS CHECK") return false;
    if (!q.type.empty() && f.type != q.type) return false;
    if (f.flowMin > q.Q || f.flowMax < q.Q) return false;
    if (f.pressureMin > q.P || f.pressureMax < q.P) return false;
    return true;
}
...
void linearSearch(const vector<Fan>& fans, const Query& q, vector<unsigned int>& out)
{
    out.clear();
    for (unsigned int i = 0; i < fans.size(); i++)
        if (matches(fans[i], q)) out.push_back(i);
}
                   ...
                   const Query& q, vector<unsigned int>& out)
{
    out.clear();
    if (!q.type.empty()) {
        if (!idx.bloomTypes.maybeContains(q.type)) return; // O(k) — точно нет
        auto it = idx.byType.find(q.type);
        if (it == idx.byType.end()) return; // ложноположительное Bloom — штатно
        for (unsigned int id : it->second)
            if (matches(fans[id], q)) out.push_back(id);
        return;
    }
    auto end_it = idx.byQmin.upper_bound(q.Q);
    for (auto it = idx.byQmin.begin(); it != end_it; ++it)
        if (matches(fans[it->second], q)) out.push_back(it->second);
}

// =====================================================================
//  РАЗДЕЛ 7: МЕНЮ — Загрузка, Поиск, Бенчмарк, Верификация
// =====================================================================
...
void printResults(const vector<Fan>& fans, const vector<unsigned int>& res, unsigned int show)
{
    ...
    if (res.empty()) return;
    unsigned int n = (unsigned int)min((size_t)show, res.size());
    cout << "\n  " << clr::DIM << "ID    Тип    Модель                  Q [м³/ч]            P [Па]          N_y kW   Статус" << clr::RST << "\n";
...
void menuLoadAndNormalize(vector<Fan>& fans, Index& idx, bool& dataReady)
{
    ...
    for (unsigned int i = 0; i < raw.size(); i++) {
        Fan f{}; f.type = raw[i].type; f.model = raw[i].model; f.size = raw[i].size;
        f.errorType = NONE; f.corrected = false; f.effImputed = false;

        unsigned int v;
        if (safeStou(removeSpaces(raw[i].id), v)) f.id = v;
        if (safeStou(removeSpaces(raw[i].diameter), v) && v < 65535) f.diameter = (unsigned short)v;
    ...
    if (dot != string::npos) outPath.insert(dot, "_normalized"); else outPath += "_normalized.csv";
    ...
    size_t memH = 0;
    for (auto& kv : idx.byType) memH += kv.second.size() * sizeof(unsigned int);
...
void menuSearch(const vector<Fan>& fans, const Index& idx)
{
    ...
    cout << "  Расход Q (м³/ч): "; string qIn; getline(cin, qIn);
...
void menuBenchmark(const vector<Fan>& fans, const Index& idx)
{
    ...
    vector<string> types;
    for (auto& kv : idx.byType) types.push_back(kv.first);
    ...
    // Сценарий А
    vector<Query> sA;
    for (unsigned int i = 0; i < N; i++) { Query q; q.type = types[typeDist(rng)]; q.Q = qDist(rng); q.P = pDist(rng); q.includeYellow = true; sA.push_back(q); }
    runScenario(fans, idx, sA, "Сценарий А: с типом (хеш-бакет + Bloom)");

    // Сценарий Б
    vector<Query> sB;
    uniform_real_distribution<double> qLow(500.0, 3000.0);
    for (unsigned int i = 0; i < N; i++) { Query q; q.type = ""; q.Q = qLow(rng); q.P = pDist(rng); q.includeYellow = true; sB.push_back(q); }
    runScenario(fans, idx, sB, "Сценарий Б: без типа (дерево по Q_min)");

    // Сценарий В
    vector<Query> sC;
    for (unsigned int i = 0; i < N; i++) { Query q; q.type = "XYZ_NOT_EXISTS"; q.Q = qDist(rng); q.P = pDist(rng); q.includeYellow = true; sC.push_back(q); }
...
// =====================================================================
//  РАЗДЕЛ 8: ВЕРИФИКАЦИЯ МАТЕМАТИЧЕСКОЙ МОДЕЛИ
// =====================================================================
...
void menuVerify(const vector<Fan>& fans, const Index& idx)
{
    ...
    if (fp_count == 0)
        cout << "    " << clr::GRN << "Гипотеза подтверждена: P_fp ≈ 0 на данной выборке ✓" << clr::RST << "\n";
    else
    ...
    for (unsigned int i = 0; i < fans.size(); i++) {
        ...
        double Q_mid = (f.flowMin + f.flowMax) / 2.0;
        double P_mid = (f.pressureMin + f.pressureMax) / 2.0;
        double N_hydr = (Q_mid * P_mid) / 3600.0;
        if (N_hydr <= 0) { r_err++; continue; }
        double r = (f.power * 1000.0) / N_hydr;
        if (r < 1.0) r_below1++;
    ...
    printf("    r ∈ [1, 1.5) : %6u (%5.1f%%)  — высокий КПД\n", r_1_15, 100.0*r_1_15/total);
    printf("    r ∈ [1.5, 10]: %6u (%5.1f%%)  — типичный диапазон\n", r_15_10, 100.0*r_15_10/total);
    printf("    r ∈ (10, 30] : %6u (%5.1f%%)  — требует проверки\n", r_10_30, 100.0*r_10_30/total);
    ...
    cout << "\n  " << clr::CYN << "3. Покрытие предикатов π₁–π₅ (до/после нормализации)" << clr::RST << "\n";
    unsigned int pi1_fix = 0, pi2_fix = 0;
    for (unsigned int i = 0; i < fans.size(); i++)
        if (fans[i].corrected) { pi1_fix++; pi2_fix++; }  // swap фиксирует обе π₁,π₂ одновременно
    ...
    printf("    π₁ (Q_min ≤ Q_max): %u записей исправлены swap\n", pi1_fix);
    printf("    π₂ (P_min ≤ P_max): %u записей исправлены swap\n", pi2_fix);
    printf("    π₃ (Q > 0):         %u нарушений после нормализации\n", pi3_fail);
    printf("    π₄ (P > 0):         %u нарушений после нормализации\n", pi4_fail);
    printf("    π₅ (N_y > 0):       %u нарушений после нормализации\n", pi5_fail);
...
int main()
{
    ...
    while (true) {
        ...
        cout << "  ║   Курсовая работа, Андреев А.А., 2025       ║\n";
        cout << "  ╚══════════════════════════════════════════════╝" << clr::RST << "\n\n";
        ...
        string choice; if (!getline(cin, choice)) break;
        choice = trim(choice);
